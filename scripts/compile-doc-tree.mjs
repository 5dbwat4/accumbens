...
const parsefy = async (importPath) => {
    ...
    if(robj.subcategories){
        result.subcategories = await Promise.all( robj.subcategories.map(async(xdir)=>{
            return await parsefy(new URL(xdir+"/",importPath))
        }))
    }else{
        result.leaf = true
    }
    ...
    const entryToData = async (entry) => {
        ...
        entryData.title = entry.title || fm.title || ""
        entryData.createdAt = new Date( entry.createdAt || fm.createdAt || fstat.birthtime).getTime()
        entryData.updatedAt = new Date( entry.updatedAt || fm.updatedAt || fstat.mtime).getTime()
        entryData.wordcount = count
        entryData.unikey = unikey
        entryData.pathonly = entry.pathonly || fm.pathonly || (
            entry === "index.md" || entry === "index.mdx"? true : false
        )
...
const config = await parsefy(new URL("./accumbens.config.js",NoteRoot_full))
